...
export default function InspectionCalendar() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      booking &&
      booking.property?.challengePrice &&
      booking.isDiscount
    ) {
  ...
  const calculateBlockedSlots = (bookingsData) => {
    ...
    bookingsData.forEach((booking) => {
      ...
      const bookingTimeIndex = allTimeSlots.findIndex(
        (slot) => slot === bookingTimeStr,
      );
      ...
      if (bookingTimeIndex !== -1) {
        ...
        for (
          let i = 0;
          i < 6 && bookingTimeIndex + i < allTimeSlots.length;
          i++
        ) {
        ...
        const next2MinuteStr = next2Hour
          .getMinutes()
          .toString()
          .padStart(2, "0");
  ...
  const parseAddress = (addressString) => {
    ...
    const addressWithoutZip = cleanedAddress
      .replace(/\b\d{5}(-\d{4})?\b/, "")
      .trim();

    const cleanedWithoutZip = addressWithoutZip.replace(/,\s*$/, "").trim();

    const parts = cleanedWithoutZip
      .split(",")
      .map((part) => part.trim())
      .filter(Boolean);
    ...
    if (streetWords.length > 0) {
      ...
      if (/^\d+$/.test(firstWord)) {
        ...
        const directionPattern =
          /^(North|South|East|West|Northeast|Northwest|Southeast|Southwest|N|S|E|W|NE|NW|SE|SW)$/i;
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const inspectionDates = await getDocs(
          collection(db, "inspectionDates"),
        );
        const inspectionDatesData = inspectionDates.docs.map((doc) =>
          doc.data(),
        );
  ...
  useEffect(() => {
    ...
    if (
      bookingData &&
      bookingData.isDiscount &&
      propertyData.paymentMethod === "challenge"
    ) {
      const mergedProperty = {
        ...propertyData,
        paymentMethod: "challenge", // Keep as challenge since URL confirms it
        challengePrice:
          bookingData.property?.challengePrice || bookingData.challengePrice,
        ...
        street:
          propertyData.street ||
          bookingData.property?.street ||
          bookingData.street ||
          "",
        streetNumber:
          propertyData.streetNumber ||
          bookingData.property?.streetNumber ||
          bookingData.streetNumber ||
          "",
        direction:
          propertyData.direction ||
          bookingData.property?.direction ||
          bookingData.direction ||
          "",
        streetName:
          propertyData.streetName ||
          bookingData.property?.streetName ||
          bookingData.streetName ||
          "",
        zip:
          propertyData.zip ||
          bookingData.property?.zip ||
          bookingData.zip ||
          "",
        address:
  ...
  const calculatePrice = (propertyData) => {
    ...
    if (bookingData && bookingData.property && paymentMethod === "challenge") {
      if (
        bookingData.isDiscount === true &&
        bookingData.property.challengePrice
      ) {
  ...
  const customDayRenderer = (day, modifiers) => {
    ...
    if (isPastDate) {
      ...
      className =
        "bg-gray-100 hover:bg-gray-200 text-gray-400 border-gray-300 opacity-50 cursor-not-allowed";
      ...
      switch (availability.status) {
        ...
        case "unavailable":
          className =
            "bg-red-100 hover:bg-red-200 text-red-800 border-red-300 opacity-50";
          break;
      }
    }
    ...
    return (
      <div
        className={`relative w-full h-full flex items-center justify-center p-2 rounded ${className}`}
      >
  ...
  const handleWaitingListSubmit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "You'll be notified within 24 hours if a slot becomes available.",
      });
    ...
    try {
      ...
      if (!selectedDate || !selectedTime) {
        ...
        toast({
          ...
          description:
            "Please select both a date and time for your inspection.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (property.paymentMethod === "pay_now") {
        ...
        const createCheckoutResponse = await fetch(
          `${functionUrl}/createCheckoutSession`,
          {
            method: "POST",
        ...
        const { checkoutUrl, error: checkoutError } =
          await createCheckoutResponse.json();
        ...
        console.log(
          "Calendar - Redirecting to Stripe Checkout (Pay Now):",
          checkoutUrl,
        );
      ...
      if (property.paymentMethod === "challenge") {
        console.log(
          "Challenge Flow - property.paymentMethod:",
          property.paymentMethod,
        );

        const approvalToken =
          localStorage.getItem("approvalToken") ||
          sessionStorage.getItem("approvalToken");

        if (!approvalToken) {
          toast({
            title: "Approval Token Missing",
            description:
              "Approval token not found. Please use the link from your email.",
            variant: "destructive",
          });
          setIsLoading(false);
          return;
        ...
        console.log(
          "Calendar - Redirecting to Stripe Checkout (Challenge):",
          checkoutUrl,
        );
        window.location.href = checkoutUrl;
        return;
  ...
  const handleBack = () => {
    ...
    if (bookingData.unitSquareFootages) {
      params.set(
        "unitSquareFootages",
        JSON.stringify(bookingData.unitSquareFootages),
      );
    }
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          >
            <div className="text-xs text-green-700 bg-white/30 p-2 rounded mt-2">
              <strong>Available:</strong> Monday-Friday (8am-5pm) starting
              tomorrow. Same-day and weekend inspections are not available.
              Real-time availability managed through inspector dashboard.
            </div>
          </GuidanceCard>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select an available date for your inspection (weekdays only,
                  starting tomorrow)
                </CardDescription>
              </CardHeader>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (blockedForDate.length > 0) {
                      setError(true);
                      setErrorMessage(
                        `${dateKey} is fully booked. All time slots are unavailable.`,
                      );
                    ...
                    if (isWeekendDate(dateValue)) {
                      toast({
                        ...
                        description:
                          "Please select a weekday (Monday-Friday) for your inspection.",
                        variant: "destructive",
                      });
                      setSelectedDate("");
                      setSelectedTime("");
                      ...
                      toast({
                        ...
                        description:
                          "This date is fully booked. Please select another date or join the waiting list.",
                        variant: "destructive",
                      });
                      setSelectedDate("");
                      setSelectedTime("");
                ...
                {selectedDate && isDateFullyBooked(new Date(selectedDate)) && (
                  <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-md">
                    <p className="text-sm text-red-800">
                      ⚠️ This date is fully booked. All time slots are
                      unavailable.
                    </p>
                    <Button
                      variant="outline"
                    ...
                    <div>
                      ...
                      {availableTimeSlots.length === 0 &&
                        timeSlots.length > 0 && (
                          <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-md">
                            <p className="text-sm text-red-800">
                              ⚠️ All time slots for this date are currently
                ...
                <CardContent className="space-y-6">
                      ...
                      {contact.occupancyStatus && (
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="capitalize">
                            Occupancy: {contact.occupancyStatus}
                          </span>
                        </div>
                      )}
                      {contact.wantsRealtorNotification &&
                        contact.realtorName && (
                          <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                            ...
                            <div className="space-y-1 text-xs text-blue-700 dark:text-blue-300">
                              ...
                              {contact.realtorEmail && (
                                <div>{contact.realtorEmail}</div>
                              )}
                              {contact.realtorPhone && (
                                <div>{contact.realtorPhone}</div>
                              )}
                            </div>
                          </div>
                        )}
                  ...
                  <div className="border-t pt-4">
                    <div className="flex justify-between items-center">
                      ...
                      >
                        {property.paymentMethod === "challenge" &&
                        property.isDiscount &&
