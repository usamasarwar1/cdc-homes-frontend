...
export default function ContactVerification({ property, onVerified, onBack }) {
  ...
  const [displaySquareFootage, setDisplaySquareFootage] = useState(
    getDisplaySquareFootage(),
  );
  const [isEditingAddress, setIsEditingAddress] = useState(false);
  const [isEditingDisplaySquareFootage, setIsEditingDisplaySquareFootage] =
    useState(false);
  ...
  const [wantsRealtorNotification, setWantsRealtorNotification] =
    useState(null);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(
        "Error parsing verified-contact-data from sessionStorage:",
        error,
      );
    }
    ...
    if (verifiedContactData) {
      ...
      setWantsRealtorNotification(
        verifiedContactData.wantsRealtorNotification || false,
      );
  ...
  const verifyCode = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://us-central1-inspection-app-4c592.cloudfunctions.net/verifyOtpCode",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!validation.isValid) {
        ...
        if (!element) {
          element = document.querySelector(
            `[data-section="${validation.elementId}"]`,
          );
        }

        if (!element) {
          element = document.querySelector(
            `input[name="${validation.elementId}"], select[name="${validation.elementId}"]`,
          );
        }
  ...
  const handleSubmit = () => {
    ...
    const params = new URLSearchParams({
      ...
      // Realtor information
      wantsRealtorNotification: (
        contactData.wantsRealtorNotification ?? false
      ).toString(),
  ...
  const isFirstNameValid =
    payeeName.firstName && payeeName.firstName.trim() !== "";
  const isLastNameValid =
    payeeName.lastName && payeeName.lastName.trim() !== "";
  const isRelationshipValid =
    relationshipToBuyer && relationshipToBuyer.trim() !== "";
  const isPhoneNumberValid =
    phoneNumber && phoneNumber.replace(/\D/g, "").length === 10;
  const isEmailValid =
    payerEmail && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payerEmail);
  const isAddressConfirmed = isAddressCorrect !== null;
  const isOccupancyStatusValid =
    occupancyStatus && occupancyStatus.trim() !== "";
  ...
  const formSections = [
    ...
    // Additional Report Recipients
    {
      ...
      isComplete:
        ...
        contactPersons.every((p) => p.firstName.trim() !== ""),
      isRequired: false,
    ...
    },
    {
      ...
      isComplete:
        contactPersons.length === 0 ||
        contactPersons.every((p) => p.lastName.trim() !== ""),
      isRequired: false,
    ...
    },
    {
      ...
      isComplete:
        contactPersons.length === 0 ||
        contactPersons.every((p) => p.email.trim() !== ""),
      isRequired: false,
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <GuidanceCard
          ...
          nextAction={
            hasInteracted
              ? "Fill out all required fields and continue"
              : "Choose an action to get started"
          }
          variant="info"
        >
          <div className="text-xs text-blue-700 bg-white/30 p-2 rounded mt-2">
            <strong>Required:</strong> Name, phone, email, relationship to
            buyer, and address confirmation
          </div>
        </GuidanceCard>
        ...
        <div className="space-y-6">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-gray-900 mb-4">
              Let's Get You Verified and Setup!
            </h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto text-center">
              Let's get you on the books, but before we do, you can preview the
              contract and see what's included, or skip and continue.
            </p>
          </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="mt-6 p-4 bg-white rounded-lg border-l-4 border-blue-500">
                  <p className="text-sm text-gray-700">
                    <strong>Professional Report:</strong> You'll receive a
                    comprehensive digital report with photos, detailed findings,
                    and recommendations within 24 hours of your inspection.
                  </p>
                </div>
                ...
                <div className="bg-white rounded-lg border p-4 mb-4">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-900">
                      CDC Home Inspections Service Agreement
                    </h3>
                    <a
                ...
                <div className="flex flex-col md:flex-row justify-center gap-4">
                  ...
                  <Button
                    onClick={() => {
                      ...
                      link.href =
                        "/attached_assets/2025 CDC Home Inspections Service Agreement_1755575856452.pdf";
                      link.download =
                        "CDC_Home_Inspections_Service_Agreement.pdf";
                      link.click();
                    }}
                    variant="outline"
          ...
          <Card
            className={
              !hasInteracted && !hasClosedModal
                ? "opacity-50 blur-sm  pointer-events-none"
                : ""
            }
            data-section="payer-info"
          >
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3" data-section="relationship-info">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
                  {["Buyer", "Realtor", "Family", "Friend", "Attorney"].map(
                    (relationship) => (
                      <Button
                        key={relationship}
                ...
                {relationshipToBuyer && relationshipToBuyer !== "buyer" && (
                  <div className="mt-3">
                    <Label htmlFor="buyerExplanation">
                      According to the Arizona Board of Technical Registration,
                      the paying party for the inspection will be listed as the
                      client. You can pay for another person, but you will be
                      listed as the client. You may list any additional report
                      recipients below.
                    </Label>
                  </div>
                )}
          ...
          {isReadyForVerification && (
            <Card
              ...
              data-section="phone-verification"
            >
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  {isVerified ? (
          ...
          >
            <CardHeader>
              ...
              <CardDescription>
                Everyone listed will receive a copy of the inspection report via
                email.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {contactPersons.length === 0 ? (
                <Button
                  ...
                  className="w-full border-2 border-blue-300 hover:border-green-400"
                >
          ...
          <Card
            ...
            data-section="realtor-notification"
          >
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                <Label>
                  Do you want your realtor to have a copy before you review the
                  report?
                </Label>
                <div className="flex gap-4">
                  <Button
                    variant={
                      wantsRealtorNotification === true ? "default" : "outline"
                    }
                  ...
                  <Button
                    variant={
                      wantsRealtorNotification === false ? "default" : "outline"
                    }
          ...
          <Card
            ...
            data-section="property-confirmation"
          >
            <CardHeader>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4 bg-blue-50 p-5 rounded-lg border-2 border-blue-300 shadow-md">
                <div className="text-center">
                  <Label className="text-xl font-bold text-blue-800">
                    REQUIRED: How Big is the House?
                  </Label>
                  <p className="text-sm text-blue-600 mt-1 font-medium">
                    Square footage is needed to calculate your inspection price
                  </p>
                </div>
              ...
              <div
                className="space-y-3 p-4 border-2 border-blue-500 rounded-lg"
                data-section="occupancy-status"
              >
                <Label className="text-base font-semibold">
                  Occupancy Status *
                </Label>
          ...
          <div
            className={`flex flex-col sm:flex-row gap-4 ${shouldBlurFormSections ? "opacity-50 blur-sm pointer-events-none" : ""}`}
          >
